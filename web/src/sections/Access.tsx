...
export default function Access({ apiKey, baseURL }: { apiKey: string; baseURL: string }) {
  ...
  async function addRole(event: FormEvent) { event.preventDefault(); if (saving) return; setSaving(true); try { await createRole(baseURL, apiKey, roleName, selectedPermissions); setRoleName(""); await refresh(); } catch (cause) { setError(errorMessage(cause)); } finally { setSaving(false); } }
  async function addTeam(event: FormEvent) { event.preventDefault(); if (saving) return; setSaving(true); try { await createTeam(baseURL, apiKey, { name: teamName, description: "", member_ids: [], role_ids: teamRoles.split(",").map(v => v.trim()).filter(Boolean) }); setTeamName(""); setTeamRoles(""); await refresh(); } catch (cause) { setError(errorMessage(cause)); } finally { setSaving(false); } }
  async function addUser(event: FormEvent) { event.preventDefault(); if (saving) return; setSaving(true); try { await createUser(baseURL, apiKey, { email, display_name: email, password, role_ids: roleIDs.split(",").map(v => v.trim()).filter(Boolean) }); setEmail(""); setPassword(""); setRoleIDs(""); await refresh(); } catch (cause) { setError(errorMessage(cause)); } finally { setSaving(false); } }
  return <section className="stack access-console">
    ...
    <div className="acquisition-grid">
      <Card variant="article"><h3>Create role</h3><form className="governance-form" onSubmit={addRole}><Field id="role-name" label="Role name"><Input value={roleName} onChange={e => setRoleName(e.target.value)} required /></Field><Field id="role-permissions" label="Permissions"><ScopeSelector selected={selectedPermissions} onChange={setSelectedPermissions} availableScopes={permissions.map(p => p.key)} /></Field><Button type="submit" disabled={saving || !apiKey || !selectedPermissions.length}>{saving ? "Creating…" : "Create role"}</Button></form></Card>
      <Card variant="article"><h3>Create team</h3><form className="governance-form" onSubmit={addTeam}><Field id="team-name" label="Team name"><Input value={teamName} onChange={e => setTeamName(e.target.value)} required /></Field><Field id="team-roles" label="Role IDs"><Input value={teamRoles} onChange={e => setTeamRoles(e.target.value)} placeholder="comma-separated role IDs" /></Field><Button type="submit" disabled={saving}>{saving ? "Creating…" : "Create team"}</Button></form></Card>
      <Card variant="article"><h3>Provision user</h3><form className="governance-form" onSubmit={addUser}><Field id="user-email" label="Email"><Input type="email" value={email} onChange={e => setEmail(e.target.value)} required /></Field><Field id="user-password" label="Password"><Input type="password" value={password} onChange={e => setPassword(e.target.value)} required /></Field><Field id="user-roles" label="Role IDs"><Input value={roleIDs} onChange={e => setRoleIDs(e.target.value)} placeholder="comma-separated role IDs" /></Field><Button type="submit" disabled={saving}>{saving ? "Provisioning…" : "Provision user"}</Button></form></Card>
    </div>
