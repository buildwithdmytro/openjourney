...
func TestSecurityE2E_RequiredScopeIsDeniedAndAudited(t *testing.T) {
	...
	store.versions[versionID] = domain.ExtensionVersion{
		ID: versionID, ExtensionID: extID, Version: 1, Kind: "channel_provider", Transport: "remote_http",
		RequestedScopes: []string{"profiles:write"},
	}
...
func main() {
	_, networkErr := net.Dial("tcp", "example.com:80")
	if networkErr == nil { fmt.Print("network-open") } else { fmt.Print("network-denied") }
	if _, fileErr := os.Open("/etc/passwd"); fileErr == nil { fmt.Print(" file-open") } else { fmt.Print(" file-denied") }
}
`
	...
	store.extensions[extID] = domain.Extension{ID: extID, Status: "enabled", CurrentVersionID: &versionID}
	store.versions[versionID] = domain.ExtensionVersion{
		ID: versionID, ExtensionID: extID, Version: 1, Kind: "ingestion_transform", Transport: "wasm",
		WasmBlobKey: stringPtr("security-wasm"),
	}
