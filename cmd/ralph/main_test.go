...
func TestCurrentMilestonePlanParsesInDocumentOrder(t *testing.T) {
	tasks, err := readTasks(filepath.Join("..", "..", "docs", "milestones", "v1-milestone-9-plan.md"))
	if err != nil {
		t.Fatal(err)
	}
	if len(tasks) != 17 {
		t.Fatalf("got %d tasks, want 17", len(tasks))
	}
	if tasks[0].ID != "14.0.1" {
		t.Fatalf("first task = %#v", tasks[0])
	}
	if tasks[len(tasks)-1].ID != "14.10.5" {
